...
from tkinter import DISABLED, END, LEFT, IntVar, Toplevel, font
...
class Interface():
    ...
    def __init__(self, master=None):
        ...
        self.button_add = tkinter.Button(self.master, image=img_add, compound=LEFT,
                                        text="Adicionar", command=self.add)
    ...
    def calculate(self):
        if len(self.archs) == 0:
            ...
            for rbd in self.archs:
                ...
                rbd.distance = pow((rbd.cost ** 2) + (rbd.unavailability ** 2), 1/2)
    ...
    def show_result(self):
        ...
        for rbd in self.archs:
            self.show_button_arch = tkinter.Button(self.frame_show_result, text=rbd.alternative,
                                                    command=lambda:self.display_value(self.archs.index(rbd)),
                                                    width=20)
            self.show_button_arch.pack()

    def display_value(self, index):
        print(self.archs[index].alternative)
        # Create the Toplevel window frame
        ...
        self.display_label_name = tkinter.Label(self.frame_display, text=self.archs[index].alternative)
        self.display_label_name['font'] = ('Calibri', '14', 'bold')
        self.display_label_name.grid(row=0, column=0)
        ...
        self.display_label_avai = tkinter.Label(self.frame_display, text=self.archs[index].availability)
        ...
        self.display_label_cost = tkinter.Label(self.frame_display, text=self.archs[index].cost)
        self.display_label_cost.grid(row=2, column=1)

        # Display unavailability
        self.display_label_unav = tkinter.Label(self.frame_display, 
                                                text="Indisponibilidade = "+ str(self.archs[index].unavailability)).grid(row=3, column=0)
        self.display_label_dist = tkinter.Label(self.frame_display, 
                                                text="Distância = " + str(self.archs[index].distance)).grid(row=4, column=0)
        self.display_label_unav_month = tkinter.Label(self.frame_display, 
                                                text="Indisp. Mês = " + str(self.archs[index].unav_month)).grid(row=5, column=0)  
        self.display_label_unav_year = tkinter.Label(self.frame_display, 
                                                text="Indisp. Ano = " + str(self.archs[index].unav_year)).grid(row=6, column=0)
